                ...
                <div class="card-header">
                    <div class="card-title">
                        @can('manage-complaint-create')
                            <a class="btn btn-primary btn-block float-end my-2" href="{{ Route('brand.create') }}">
                                    ...
                                    <tbody>
                                        @if(isset($brands) && $brands)
                                            ...
                                            @foreach ($brands as $value)
                                                @php $i++;  @endphp
                                                <tr>
                                                    <td class="gridjs-td" data-column-id="Brand">{{ $value->name }}</td>
                                                    <td class="gridjs-td" data-column-id="Action">
                                                        {{-- @can('manage-complaint-edit') --}}
                                                            <a class="btn btn-sm btn-icon btn-info-light rounded-circle" href="{{Route('brand.edit',$value->id)}}"> <i class="fa fa-pencil" aria-hidden="true"></i> </a>
                                                        {{-- @endcan
...
<script>
    function deleteFunction(id){
        $('#deleteForm').attr('action','{{ url("brand") }}'+ '/'+id);
