                ...
                <div class="card-header">
                    <div class="card-title">
                        @can('manage-complaint-create')
                            <a class="btn btn-primary btn-block float-end my-2" href="{{ Route('group.create') }}">
                                    ...
                                    <tbody>
                                        @if(isset($groups) && $groups)
                                            ...
                                            @foreach ($groups as $value)
                                                @php $i++;  @endphp
                                                <tr>
                                                    <td class="gridjs-td" data-column-id="Group">{{ $value->name }}</td>
                                                    <td class="gridjs-td" data-column-id="Action">
                                                        {{-- @can('manage-complaint-edit') --}}
                                                            <a class="btn btn-sm btn-icon btn-info-light rounded-circle" href="{{Route('group.edit',$value->id)}}"> <i class="fa fa-pencil" aria-hidden="true"></i> </a>
                                                        {{-- @endcan
...
<script>
    function deleteFunction(id){
        $('#deleteForm').attr('action','{{ url("group") }}'+ '/'+id);
