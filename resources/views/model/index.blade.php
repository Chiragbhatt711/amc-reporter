...
<div class="page-header d-flex align-items-center justify-content-between border-bottom mb-4">
    <h1 class="page-title">Manage Model</h1>
    {{-- <div>
        <ol class="breadcrumb">
                ...
                <div class="card-header">
                    <div class="card-title">
                        @can('manage-complaint-create')
                            <a class="btn btn-primary btn-block float-end my-2" href="{{ Route('model.create') }}">
                                    ...
                                    <tbody>
                                        @if(isset($models) && $models)
                                            ...
                                            @foreach ($models as $value)
                                                @php $i++;  @endphp
                                                <tr>
                                                    <td class="gridjs-td" data-column-id="Model">{{ $value->name }}</td>
                                                    <td class="gridjs-td" data-column-id="Action">
                                                        {{-- @can('manage-complaint-edit') --}}
                                                            <a class="btn btn-sm btn-icon btn-info-light rounded-circle" href="{{Route('model.edit',$value->id)}}"> <i class="fa fa-pencil" aria-hidden="true"></i> </a>
                                                        {{-- @endcan
...
<script>
    function deleteFunction(id){
        $('#deleteForm').attr('action','{{ url("model") }}'+ '/'+id);
